...
document.getElementById('nextBtn2').addEventListener('click', () => {
    ...
    // Extract resort name from file name (assuming filename follows "ResortName.csv")
    const resortName = file.name.split('.')[0].trim(); // Get the filename without the extension

    // Read CSV file and save to Firebase
    const reader = new FileReader();
    reader.onload = function (event) {
        const csvData = event.target.result;
        saveCsvDataToFirebase(resortName, csvData); // Save data to Firebase under the matched resort
    };
    reader.readAsText(file);

    // Prepare and send file to server for sentiment analysis
...
// Save CSV data to Firebase under the matched resort name
function saveCsvDataToFirebase(resortName, csvData) {
    const lines = csvData.split("\n").map(line => line.split(",")); // Split CSV into rows and columns
    ...
    const jsonData = lines.slice(1).map(row => {
        if (row.length < 3) return null; // Ensure valid row format (Comment, Predicted Sentiment, True Sentiment)
        return {
            Comments: row[0].trim(),
            "Predicted Sentiment": row[1].trim(),
            "True Sentiment": row[2].trim(),
        };
    }).filter(Boolean); // Remove invalid entries

    // Reference to the matching resort in "reviews/"
    const csvRef = ref(db, `reviews/${resortName}/csvData`);

    // Save data under the matched resort name
    jsonData.forEach(data => {
        push(csvRef, data)
            .then(() => console.log(`CSV data saved successfully for resort: ${resortName}`))
            .catch(error => console.error('Error saving CSV data to Firebase:', error));
    });
}
